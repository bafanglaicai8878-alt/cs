...
from database import DOC_SUDAMA, read_json_cache, write_json_cache

CURRENT_VERSION = "1.64p1"  # 当前版本号
GITHUB_REPO = "bafanglaicai8878-alt/cs"
...
DEFAULT_CONFIG = {
    ...
    "Github_Personal_Tokens": [],
    "Steam_Web_API_Key": "",
    "Full_Steam_Catalog": True,
    "Only_Importable_Games": True,
    "Catalog_Merge_On_Sync": True,
    "Catalog_Auto_Sync_Enabled": True,
    "Catalog_Auto_Sync_Hours": 24,
    "Catalog_Github_Max_Pages": 0,
    "Custom_Steam_Path": "",
    "Force_Unlocker": "",
    "Box_Server_URL": "",
...
class STConverter:
    ...
    def pack_lua(self, lua_content: str, time_val: int | None = None) -> bytes:
        if time_val is None:
            time_val = random.randint(0, 0xFFFFFFFF)
        key = time_val & 0xFF
        payload = b'\x00' * 512 + lua_content.encode('utf-8')
        ...
        comp_size = len(enc)
        dc_size = len(payload)
        enc_time = (time_val ^ self.KEY1) & 0xFFFFFFFF
        enc_dc_size = ((dc_size - self.SIZE_OFFSET) ^ self.KEY1) & 0xFFFFFFFF
        header = struct.pack('<III', enc_time, comp_size, enc_dc_size)
        return header + bytes(enc)

    def pack_file(self, lua_file_path: str, st_file_path: str | None = None) -> Path:
        lua_path = Path(lua_file_path)
        st_path = Path(st_file_path) if st_file_path else lua_path.with_suffix('.st')
        lua_content = lua_path.read_text(encoding='utf-8')
        st_path.write_bytes(self.pack_lua(lua_content))
        return st_path
...
class CaiBackend:
    def __init__(self):
        ...
        self._github_token_idx = 0

    def get_github_tokens(self) -> List[str]:
        return normalize_github_tokens(self.config or {})

    def current_github_token(self) -> str:
        tokens = self.get_github_tokens()
        if not tokens:
            return ""
        return tokens[self._github_token_idx % len(tokens)]

    def github_auth_headers(self, token: str = "") -> Dict[str, str] | None:
        t = (token or self.current_github_token()).strip()
        return {"Authorization": f"Bearer {t}"} if t else None

    async def select_best_github_token(self) -> bool:
        """同步开始前选用剩余配额最多的 Token。"""
        tokens = self.get_github_tokens()
        if not tokens:
            return False
        best_idx = 0
        ...
        for i, t in enumerate(tokens):
            rem, _ = await self.get_github_rate_info(token=t)
            if rem > best_rem:
                best_rem = rem
                best_idx = i
        self._github_token_idx = best_idx
        if len(tokens) > 1:
            self.log.info(
                f"已选择 GitHub Token #{best_idx + 1}/{len(tokens)}（剩余 {best_rem} 次）"
            )
        return best_rem > 0

    async def rotate_github_token(self) -> bool:
        tokens = self.get_github_tokens()
        if len(tokens) <= 1:
            return False
        for step in range(1, len(tokens)):
            idx = (self._github_token_idx + step) % len(tokens)
            remaining, _ = await self.get_github_rate_info(token=tokens[idx])
            if remaining >= 1:
                self._github_token_idx = idx
                self.log.info(
                    f"切换 GitHub Token #{idx + 1}/{len(tokens)}（剩余 {remaining} 次）"
                )
                return True
        return False
    ...
    async def check_for_updates(self) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            github_token = self.current_github_token()
    ...
    def finalize_steamtools_plugin(self, lua_filepath: Path) -> None:
        if not self.is_steamtools() or not lua_filepath.exists():
            return
        try:
            st_path = STConverter().pack_file(str(lua_filepath))
            self.log.info(f"已为 SteamTools 编译插件: {st_path.name}")
        except Exception as e:
            self.log.error(f"编译 .st 插件失败: {self.stack_error(e)}")
    ...
    async def load_config(self) -> Dict | None:
        ...
        try:
            async with aiofiles.open("./config.json", mode="r", encoding="utf-8") as f:
                ...
                config['Github_Personal_Tokens'] = normalize_github_tokens(config)
                if config['Github_Personal_Tokens']:
                    config['Github_Personal_Token'] = config['Github_Personal_Tokens'][0]
    ...
    def get_all_github_repos(self) -> List[str]:
        """获取所有GitHub仓库（内置+自定义）"""
        builtin_repos = ['luomojim/ManifestAutoUpdate', 'Auiowu/ManifestAutoUpdate', 'SteamAutoCracks/ManifestHub']
    ...
    # --- MODIFIED: Simplified to use the new caching logic ---
    async def _get_sudama_data(self) -> Dict[str, str]:
        """从 sudama API 获取所有密钥数据 (Sudama库模式调用)"""
        return await self._get_cached_sudama_data()
    ...
    async def patch_lua_with_depotkey(self, app_id: str, lua_file_path: Path) -> bool:
        ...
        try:
            if self.is_steamtools():
                self.log.warning(
                    "SteamTools 模式下跳过创意工坊 depotkey 修补（避免破坏 addappid 格式）"
                )
                return False
...
# 在 backend.py 中替换 _download_and_place_workshop_manifest 方法
    ...
    async def get_github_rate_info(self, token: str = "") -> tuple[int, int]:
        """返回 (remaining, reset_unix)。"""
        headers = self.github_auth_headers(token)
        try:
            r = await self.client.get("https://api.github.com/rate_limit", headers=headers, timeout=15)
            r.raise_for_status()
            core = r.json().get("resources", {}).get("core", {})
            return int(core.get("remaining", 0)), int(core.get("reset", 0))
        except Exception:
            return 0, 0

    async def wait_github_rate_limit(self, min_remaining: int = 5, max_wait: int = 3700) -> bool:
        """等待或轮换 Token 直到有足够配额。"""
        tokens = self.get_github_tokens()
        while True:
            remaining, reset_at = await self.get_github_rate_info()
            if remaining >= min_remaining:
                return True
            if await self.rotate_github_token():
                continue
            if not reset_at:
                return False
            wait_sec = min(max_wait, max(1, reset_at - int(time.time()) + 2))
            reset_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(reset_at))
            if tokens:
                self.log.warning(
                    f"全部 {len(tokens)} 个 Token 配额不足，等待 {wait_sec} 秒至 {reset_time} 后继续…"
                )
            else:
                self.log.error(
                    f"GitHub API 已用尽（无 Token 仅 60 次/小时），将于 {reset_time} 重置。"
                )
                if wait_sec > 120:
                    return False
    ...
    async def check_github_api_rate_limit(self) -> bool:
        ...
        if await self.rotate_github_token():
            return True
        reset_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(reset_at)) if reset_at else "未知"
        self.log.error(f"GitHub API 请求次数已用尽，将于 {reset_time} 重置。")
        if not self.get_github_tokens():
            self.log.error("请在后台配置 Github Token 以提升上限至 5000/小时/Token。")
        return False
    ...
    async def _get_depots(self, appid: str) -> List[Dict]:
        ...
        raw = await self._get_depots_safe(appid)
        if not raw:
            return []
        ...
        for item in raw:
            if isinstance(item, dict):
                ...
                continue
            try:
                depot_id, manifest_id, size, source = item
            except (TypeError, ValueError):
                continue
            dlc_appid = None
            if isinstance(source, str) and source.startswith("DLC:"):
    ...
    async def _process_zip_manifest_generic(self, app_id: str, download_url: str, source_name: str, add_all_dlc: bool = False, patch_depot_key: bool = False) -> bool:
        ...
        try:
            ...
            if self.is_steamtools():
                ...
                self.finalize_steamtools_plugin(lua_filepath)
    ...
    # --- MODIFIED: Replaced with Sudama API + Caching logic ---
    async def _get_cached_sudama_data(self) -> Dict:
        """
        核心函数：获取 Sudama 密钥数据
        逻辑：检查缓存 -> 检查时间戳是否超过24小时 -> 下载或读取缓存
        """
        cache_file = Path("sudama_cache.json")
        current_time = time.time()
        one_day_seconds = 86400

        # 1. 尝试读取缓存
        try:
            cache_content = read_json_cache(DOC_SUDAMA, cache_file)
            if cache_content:
                last_update = cache_content.get("timestamp", 0)
                if current_time - last_update < one_day_seconds:
                    self.log.info(
                        f"使用本地缓存的密钥库 (上次更新: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(last_update))})"
                    )
        ...
        try:
            ...
            write_json_cache(DOC_SUDAMA, cache_data, cache_file)
    ...
    async def process_sudama_manifest(self, app_id: str, add_all_dlc: bool = False, patch_depot_key: bool = False) -> bool:
        """处理 Sudama 库清单"""
        try:
            self.log.info(f'正从 Sudama 库处理 AppID {app_id} 的清单...')
            
            # 1. 获取 Depot 和 Manifest 信息 (使用现有的 SteamUI/DDXNB 接口)
            # 这与 manifest(2) 库的逻辑一致，先要有结构，再去填密钥
            depot_manifest_map = await self._get_depots_and_manifests_from_steamui(app_id)
            if not depot_manifest_map:
            ...
            # 2. 获取 Sudama 的所有密钥数据
            sudama_keys = await self._get_sudama_data()
            if not sudama_keys:
                self.log.error("无法获取 Sudama 密钥库数据")
                return False

            # 3. 匹配 Depot 与 Key (在 sudama_keys 中搜索)
            valid_depots = {}
            ...
            if not valid_depots:
                ...
                return False
            missing_depots = [d for d in depot_manifest_map.keys() if d not in valid_depots]
            if missing_depots:
                self.log.warning(
                    f"AppID {app_id} 的 Sudama 密钥不完整："
                    f"{len(valid_depots)}/{len(depot_manifest_map)}，缺少 {', '.join(missing_depots[:8])}"
                )
                return False

            # 4. 根据解锁工具类型处理 (复用 ManifestHub V2 的处理逻辑，因为数据结构已经对其了)
            # 这里复用 _process_steamautocracks_v2_... 系列函数，因为它们本质上就是接收 (depot_map, valid_keys) 然后生成文件
            # 同时也继承了自动更新(st_auto_update)的逻辑
            if self.is_steamtools():
                # 注意：这里我们把 sudama_keys 当作 depotkeys_data 传入，以便复用修补创意工坊密钥的逻辑
                return await self._process_steamautocracks_v2_for_steamtools(
                    app_id, valid_depots, depot_manifest_map, add_all_dlc, patch_depot_key, sudama_keys
                )
            else:
    ...
    async def process_steamautocracks_v2_manifest(self, app_id: str, add_all_dlc: bool = False, patch_depot_key: bool = False) -> bool:
        ...
        try:
            ...
            # 1. 从 steamui API 获取 depot 和 manifest 信息 (内置备用API)
            depot_manifest_map = await self._get_depots_and_manifests_from_steamui(app_id)
            if not depot_manifest_map:
                self.log.error(f"未能从主API或备用API获取到 AppID {app_id} 的 depot 信息,请检查APP ID是否正确或API请求问题")
                return False
            ...
            if not valid_depots:
                self.log.warning(f"AppID {app_id} 没有找到任何有效的 depot 密钥,此APP ID可能未收录")
                return False
            missing_depots = [d for d in depot_manifest_map.keys() if d not in valid_depots]
            if missing_depots:
                self.log.warning(
                    f"AppID {app_id} 的 ManifestHub(2) 密钥不完整："
                    f"{len(valid_depots)}/{len(depot_manifest_map)}，缺少 {', '.join(missing_depots[:8])}"
                )
                return False
    ...
    async def _get_depots_and_manifests_from_ddxnb(self, app_id: str) -> Dict[str, str]:
        ...
        try:
            ...
            if not app_data or "depots" not in app_data:
                self.log.error(f"备用API响应中未找到 AppID {app_id} 的 depots 信息")
                return {}
            ...
            if depot_manifest_map:
                ...
                self.log.warning(f"备用API未找到 AppID {app_id} 的任何有效 depot-manifest 映射,请检查APP ID是否正确")

            return depot_manifest_map

        except Exception as e:
            self.log.error(f"从备用API (steam.ddxnb.cn) 获取 depot 信息失败,请检查APP ID是否正确或API请求问题: {e}")
            return {}

    async def _get_depots_and_manifests_from_steamcmd(self, app_id: str) -> Dict[str, str]:
        """从 SteamCMD API 获取 depot 和 manifest 映射。"""
        try:
            self.log.info(f"正在尝试 SteamCMD API 获取 AppID {app_id} 的信息...")
            response = await self.client.get(f"https://api.steamcmd.net/v1/info/{app_id}", timeout=20)
            response.raise_for_status()
            data = response.json()

            app_data = data.get("data", {}).get(str(app_id), {})
            depots = app_data.get("depots", {})
            depot_manifest_map: Dict[str, str] = {}

            for depot_id, depot_info in depots.items():
                if not str(depot_id).isdigit() or not isinstance(depot_info, dict):
                    continue
                manifests = depot_info.get("manifests", {})
                public_manifest = manifests.get("public", {}) if isinstance(manifests, dict) else {}
                manifest_id = public_manifest.get("gid") if isinstance(public_manifest, dict) else None
                if manifest_id:
                    depot_manifest_map[str(depot_id)] = str(manifest_id)
                    self.log.info(f"从 SteamCMD API 发现有效 depot: {depot_id}, manifest: {manifest_id}")

            if depot_manifest_map:
                self.log.info(f"从 SteamCMD API 总共找到 {len(depot_manifest_map)} 个 depot")
            else:
                self.log.warning(f"SteamCMD API 未找到 AppID {app_id} 的有效 depot-manifest 映射")
            return depot_manifest_map
        except Exception as e:
            self.log.error(f"从 SteamCMD API 获取 depot 信息失败: {e}")
            return {}
    ...
    async def _get_depots_and_manifests_from_steamui(self, app_id: str) -> Dict[str, str]:
        ...
        try:
            ...
            self.log.warning(f"主API (steamui.com) 访问或解析失败: {e},请检查APP ID是否正确或API请求问题。")
            if vdf_content:
                self.log.warning(f"主API返回内容预览: {vdf_content[:300]}...")
            self.log.info("正在尝试备用API (steam.ddxnb.cn)...")
        
        # 2. 如果主API失败，调用备用API
        depot_manifest_map = await self._get_depots_and_manifests_from_ddxnb(app_id)
        if depot_manifest_map:
            return depot_manifest_map

        # 3. 再尝试 SteamCMD API
        self.log.info("备用API失败，正在尝试 SteamCMD API...")
        return await self._get_depots_and_manifests_from_steamcmd(app_id)
    ...
    async def _patch_lua_with_existing_depotkeys(self, app_id: str, lua_file_path: Path, depotkeys_data: Dict) -> bool:
        ...
        try:
            ...
            if app_id not in depotkeys_data:
                self.log.warning(f"没有此AppID的depotkey: {app_id},这是正常情况，此游戏可能没有创意工坊密钥或者暂未收录，不影响本体使用")
                return False
            ...
            # 检查 depotkey 是否有效
            if not depotkey or not str(depotkey).strip():
                self.log.warning(f"AppID {app_id} 的 depotkey 为空或无效，跳过修补: '{depotkey}，这是正常情况，此游戏可能没有创意工坊密钥，或者暂未收录，不影响本体使用'")
                return False
    ...
    async def search_all_repos_for_appid(self, app_id: str, repos: List[str] = None) -> List[Dict]:
        ...
        headers = self.github_auth_headers()
    ...
    async def find_appid_by_name(self, game_name: str) -> List[Dict]:
        try:
            ...
            if isinstance(data, list) and len(data) > 0:
                self.log.info(f"成功找到 {len(data)} 个结果")
                return data
            else:
